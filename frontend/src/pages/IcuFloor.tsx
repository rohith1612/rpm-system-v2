...
const BedCard = ({ bedId, patientId, alerts, now, onBedClick, onUnassign, onDelete }: any) => {
  const patient = useAppStore(state => state.patients[patientId]);
  
  if (!patient) return null;

  const isLive = isPatientActive(patient, now);
  const isCritical = alerts.some((a: any) => a.patient_id === patientId && a.severity === 'critical');

  const getVitalAlertSeverity = (vitalKey: string): string | null => {
    const alert = alerts.find((a: any) => a.patient_id === patientId &&
  ...
  if (!isLive) statusColor = 'var(--ink)';
  else if (isCritical) statusColor = 'var(--red)';

  return (
    <div className={`bed ${isLive ? 'live' : ''} ${isCritical ? 'critical' : ''}`} onClick={() => onBedClick(bedId)} style={{ cursor: 'pointer' }}>
      <div className="bed-top">
        ...
        <div className="bed-tags">
          ...
          <span className="bed-x" title="Unassign Patient" onClick={(e) => onUnassign(e, bedId)}>&times;</span>
          <span className="bed-x" title="Delete Bed" onClick={(e) => onDelete(e, bedId)} style={{ marginLeft: '4px' }}>
...
export default function IcuFloor() {
  ...
  const handleDeleteBed = async (e: React.MouseEvent, bedId: string) => {
    e.stopPropagation();
    if (bedMap[bedId]) {
      try { await unassignBed(bedId); } catch (err) {}
    }
    ...
    <div className="content" id="icuView">
      ...
      <div className="bed-floor">
        {bedsList.map((bedId) => {
          ...
          if (patientId) {
            return <BedCard key={bedId} bedId={bedId} patientId={patientId} alerts={alerts} now={now} onBedClick={handleBedClick} onUnassign={handleUnassign} onDelete={handleDeleteBed} />;
          }
