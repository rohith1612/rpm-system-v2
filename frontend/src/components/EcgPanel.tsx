...
import { fetchPatientEcg } from "../api";
...
export default function EcgPanel({ patient, selectedVital, onSelectVital, onExpand, isDataStale }: Props) {
  ...
  const { latestEcg, setLatestEcg } = useAppStore();
  const storeEcg = latestEcg[patient.id];
  
  // Use store ECG if available, fallback to patient prop
  ...
  useEffect(() => {
    if (patient) {
      ...
      // Fetch latest ECG buffer
      fetchPatientEcg(patient.id).then(buffer => {
        if (Array.isArray(buffer) && buffer.length > 0) {
           const latest = buffer[buffer.length - 1];
           if (latest && latest.data) {
             setLatestEcg(patient.id, latest.data);
           }
        }
      }).catch(err => console.error("Failed to fetch ECG buffer:", err));
    }
  }, [patient?.id, setLatestEcg]);
      ...
      <div className="ecg-panel__content-row">
        ...
        <div className="ecg-panel__params">
          <div className="ecg-panel__rhythm">
            ...
            <span className="ecg-panel__rhythm-value" style={{ color: isDataStale ? 'var(--slate-400)' : undefined }}>
              {isDataStale ? "--" : (ecg as any)?.rhythm || "Normal Sinus Rhythm"}
            </span>
          </div>
          {PARAM_LABELS.map(({ key, label, unit, format }) => {
            const raw = ecg ? (ecg as any)[key] : null;
        ...
        <div className="ecg-panel__waveforms">
          <div className="ecg-panel__waveform-main glass-card">
            <EcgWaveform ecg={ecg as any} patient={patient} waveType="ecg" lead="II" isDataStale={isDataStale} />
          </div>
          <div className="ecg-panel__waveform-row">
            <div className="ecg-panel__waveform-sub glass-card">
              <EcgWaveform ecg={ecg as any} patient={patient} waveType="pleth" isDataStale={isDataStale} />
            </div>
            <div className="ecg-panel__waveform-sub glass-card">
              <EcgWaveform ecg={ecg as any} patient={patient} waveType="resp" isDataStale={isDataStale} />
            </div>
          </div>
        </div>
      </div>
