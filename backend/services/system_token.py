...
from backend.config import CERNER_TOKEN_URL, SYSTEM_CLIENT_ID, SYSTEM_SECRET, SYSTEM_SCOPES
...
async def get_system_token() -> str:
    ...
    if not SYSTEM_CLIENT_ID or not SYSTEM_SECRET:
        raise ValueError("System credentials (SYSTEM_CLIENT_ID, SYSTEM_SECRET) are not configured.")
