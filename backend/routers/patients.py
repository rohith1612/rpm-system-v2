...
async def search_cerner_patients(query: str, _token: str = Depends(require_cerner_auth)):
    ...
    for entry in entries:
        ...
        full_name = (
            names.get("text")
            or f"{names.get('given', [''])[0]} {names.get('family', '')}"
        )
    ...
    if results:
        encounter_results = await asyncio.gather(
            *(check_encounter(p["id"]) for p in results)
        )
...
async def get_cerner_patient(cerner_patient_id: str, _token: str = Depends(require_cerner_auth)):
    ...
    names = res.get("name", [{}])[0]
    full_name = (
        names.get("text") or f"{names.get('given', [''])[0]} {names.get('family', '')}"
    )
    ...
    try:
        ...
        if enc_resp.status_code == 200:
            ...
            for entry in enc_data.get("entry", []):
                ...
                enc_type_text = enc_type_list[0].get("text") or (
                    enc_type_list[0].get("coding", [{}])[0].get("display")
                    if enc_type_list[0].get("coding")
                    else None
                )
...
async def create_new_patient(patient: PatientCreateUpdate, _token: str = Depends(require_auth)):
    ...
    from backend.services.vitals_service import (create_patient,
                                                 get_latest_vitals_map)

    try:
        res = create_patient(
            patient.patient_id, patient.name, patient.age, patient.condition
        )
...
def get_patient_insights(patient_id: str, _token: str = Depends(require_auth)):
    ...
    if not vitals:
        return {
            "error": "Not enough data. Please wait until at least 10 minutes of telemetry has been collected."
        }, 400
    ...
    if time_span_seconds < 600:
        return {
            "error": f"Not enough data. Only {int(time_span_seconds // 60)} minutes collected. Minimum 10 minutes required."
        }, 400
